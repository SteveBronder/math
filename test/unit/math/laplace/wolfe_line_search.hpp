...
struct QuadraticObjective {
  ...
  QuadraticObjective(const Eigen::MatrixXd& Q_in, const Eigen::VectorXd& b_in,
                     double scale_in = 1.0)
      : Q(Q_in), b(b_in), scale(scale_in) {}
...
/**
 * @brief Linear log-likelihood used to verify autodiff propagation.
 */
struct LinearLogLikelihood {
  template <typename Vec, typename... Args>
  auto operator()(const Vec& theta, const var& coeff, Args&&...) const {
    using scalar_t = typename std::decay_t<Vec>::Scalar;
    Eigen::Matrix<scalar_t, Eigen::Dynamic, 1> theta_cast = theta;
    return coeff * theta_cast.sum();
  }
};

// Checks that gradients for ll_args propagate when the Wolfe step succeeds.
TEST(WolfeLineSearch, AutodiffGradientsPropagateOnSuccess) {
  ...
  LinearLogLikelihood ll_fun;
  auto ll_args = std::make_tuple(coeff);

  auto status = harness.run(info, obj, ll_fun, ll_args);
  EXPECT_EQ(status.stop_, WolfeReturn::Wolfe);

  double expected_coeff_adj = info.curr_.theta_.sum();
  EXPECT_NEAR(coeff.adj(), expected_coeff_adj, 1e-10);
  Eigen::VectorXd expected_theta_grad = Eigen::VectorXd::Ones(2) * coeff.val();
  EXPECT_TRUE(info.curr_.theta_grad_.isApprox(expected_theta_grad, 1e-10));
  recover_memory();
}
...
// Checks that precheck failures return StepTooSmall.
